...
from W_Alg_WRegu_UpdateA import *
from W_Alg_L1 import *
...
class simulateOnlineData():
	def __init__(self, dimension, iterations, articles, users, 
		...
		W0 = self.W0.copy()
		self.GW0 = self.initializeGW0()
		#self.GW0 = self.getGW0()
		self.noiseLevel = noiseLevel
		self.matrixNoiseLevel = matrixNoiseLevel
		#print 'ZZZZZZZZZZZ', self.W
	...
	def runAlgorithms(self, algorithms):
		...
		filenameWriteRegret = os.path.join(save_address, 'AccRegret' +fileSig + timeRun_Save + '.csv')
		filenameWritePara = os.path.join(save_address, 'ParameterEstimation' + fileSig+ timeRun_Save + '.csv')
		...
		for iter_ in range(self.iterations):
			...
			if iter_%self.batchSize == 0:
				...
				for alg_name in algorithms.iterkeys():
					...
					CurrentRegret = sum([u[-1] for u in AccRegret[alg_name].itervalues()])
					BatchAverageRegret[alg_name].append(CurrentRegret)
					'''
...
if __name__ == '__main__':
	...
	for i in range(1):
		...
		if algName == 'LinUCB':
			...
			#algorithms['LinUCB_useOptPackage'] = LinUCBAlgorithm_G(dimension = dimension, alpha = alpha, lambda_ = lambda_, n = n_users, RankoneInverse = RankoneInverse)
		...
		if algName =='CoLin':
			...
			#algorithms['CoLin_Package'] = CoLinAlgorithm_G(dimension=dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW0(), RankoneInverse = RankoneInverse)
		...
		if algName =='GOBLin_TrueW':
			algorithms['GOBLin_TrueW'] = GOBLinAlgorithm( dimension= dimension, alpha = G_alpha, lambda_ = G_lambda_, n = n_users, W = simExperiment.getGW(), RankoneInverse = RankoneInverse )
		...
		if algName =='ALL':
			...
			#algorithms['LearnW_ProvideGradient'] =  LearnWAlgorithm_G(dimension = dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW0(), windowSize = 2, RankoneInverse=RankoneInverse)
			algorithms['LearnW_UpdateA'] =  LearnWAlgorithm_G_UpdateA(dimension = dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW0(), windowSize = 2, RankoneInverse=RankoneInverse)

			#algorithms['LearnW_l1'] =  LearnWAlgorithm_l1(dimension = dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW0(), windowSize = 2, RankoneInverse=RankoneInverse)
			#algorithms['LearnW_l1_G'] =  LearnWAlgorithm_l1_G(dimension = dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW0(), windowSize = 2, RankoneInverse=RankoneInverse)
			algorithms['LearnW_l1_G_UpdaetA'] = LearnWAlgorithm_l1_G_UpdateA(dimension = dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW0(), windowSize = 2, RankoneInverse=RankoneInverse)
			#algorithms['LearnW_RegW_UpdaetA'] = LearnWAlgorithm_G_WRegu_UpdateA(dimension = dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW0(), windowSize = 2, RankoneInverse=RankoneInverse)
		...
		if algName == 'LearnW':
			algorithms['LearnW_ProvideGradient_ReguId'] =  LearnWAlgorithm_G(dimension = dimension, alpha = alpha, lambda_ = lambda_, n = n_users, W = simExperiment.getW0(), windowSize = 2, RankoneInverse=RankoneInverse)
                if algName == 'LearnW_TrueW':
                        algorithms['LearnW_TrueW'] = LearnWAlgorithm_G(dimension = dimension, alpha = alpha, lambda_ =lambda_, n = n_users, W = simExperiment.getW(), windowSize = 2, RankoneInverse = RankoneInverse)
                if algName == 'LearnW_NoGradient':
                        algorithms['LearnW_NoGradient'] = LearnWAlgorithm(dimension = dimension, alpha = alpha, lambda_= lambda_, n = n_users, W= simExperiment.getW0(), windowSize = 2, RankoneInverse = RankoneInverse)
